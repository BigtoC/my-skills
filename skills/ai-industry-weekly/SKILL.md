...
**实测能力（会变，必须现看 `coverage` 栏，别照抄任何一次的数字）**：

| 实测日 | DRAM | LYTE | NCLD | SMH | SOXX |
|---|---|---|---|---|---|
| 2026-09-03 | 5 | **0** | **0** | 10 | 10 |
| 2026-09-07 | 4 | **5** | **5** | 9 | 10 |

⚠️ **早先这里写「LYTE 0 笔、NCLD 0 笔 …… 一旦 AV 取不到就直接掉到第三级，只能人工翻官网」，
并注明「勿再重新调查」——那句话现在是错的。** 2026-09-07 实跑 LYTE / NCLD 各回 5 笔，
DRAM 与 SMH 反而比四天前少一笔。yfinance 对新 ETF 的覆盖是**上游随时会动的东西**，
不是可以测一次就钉死的常数。

**所以判定一律看脚本每档输出的 `coverage`（`top-N` / `none`），不要依赖本表任何一行。**
本表只用来说明一件事：**这一级的档数会变，且变化方向不可预测**。
派生量能不能算，由 `derived.na_reasons` 逐项说明（N<10 就没有前十大，全量项永远 N/A）。
...
脚本报「日配额耗尽」时**无法区分「这个 key 用完了」与「这个 key 打错了」**——Alpha Vantage 对无效 key 与配额耗尽的 key 返回**完全相同**的消息。两种情形都会被当成耗尽、换下一个 key。

**实测（2026-09-05）：`ETF_PROFILE` 端点根本不校验 key，`--check` 因此证伪不了拼错的 key。**
`AV_API_KEYS=BOGUSKEY123 python3 "$SKILL_DIR/scripts/etf_holdings.py" --check` 回 **「✓ 可用（DRAM 返回 26 笔持仓）」、结论「1/1 个 key 当前可用」、退出码 0**；同日直接 curl 同一端点、换两个不同的假 key，一样回完整 26 笔持仓（`GLOBAL_QUOTE` 同测亦然）。由此：

- **`--check` 通过 ≠ key 有效。** 它只证明「此刻取得到数」，不证明这串 key 是真的；一个打错的 key 会**通过** `--check`，不会被它拦下来。要确认 key 本身，只能另找一个会校验 key 的端点，本脚本做不到。
- 上一段那句「无法区分」依然成立，但**「若所有 key 都在第一次调用就报耗尽，先怀疑 key 拼错」这条推断已被本次实测推翻，不要再照它排查**：打错的 key 不报耗尽，它回真数据。所有 key 首次调用就报耗尽，指向的是限流／配额本身（含瞬时限流被误判成日配额的可能——瞬时限流的文案里也含 "per day"，判定顺序见 `scripts/etf_holdings.py` 档头「限流处置」第 2 条），不是拼写。
- 因此拼错 key 的实际症状**不是取不到数**，而是这一步**看起来完全正常**——报告照常拿到持仓数字，你却无从判断它是不是走了自己的配额。这条只能记录，本步无法自检。
...
#### ⚠️ 口径红线一之二：全量层也有同名不同义 —— `top3` 与 `top3_merged` 差 23pt

上面那条讲的是 top-N 层的陷阱。**全量层（Alpha Vantage）有它自己的一个**，而且更隐蔽，
因为这一级 `coverage=full`、`degraded=false`，看起来一切正常。

带 swap 的档（三档 Roundhill）在 AV 的持仓表里，**同一个标的会占两行**：实物一行、
`… SWAP NM` 一行。脚本因此产出两个数：

| 字段 | DRAM 实测 2026-09-07 | 含义 |
|---|---|---|
| `derived.top3` | **0.5029** | 逐行原始——三星、SK海力士、`MICRON … SWAP NM` 各算一行 |
| `derived.top3_merged` | **0.7298** | 同一发行人的 swap 行与实物行**合并后** |

`references/rating-rules.md` 正文人工核过的「前三大(三星/SK海力士/美光)**≈73%**」
对应的是 **`top3_merged`**（差 0.0pt）。**引 `top3` 会低报 22.7pt**——与上面 FGXXX 那个
「差 22pt 却同名」的量级一样，只是这次错在**合并口径**而不是现金行项目。

**所以：「前三大／前十大＝几档股票占比」一律引 `*_merged`；`top3` / `top10` 只在**
**你要的确实是「逐行」口径时才用，并写明是逐行。** 两者都不带 swap 标记，
光看数字分不出来，只能靠字段名。

**现金类同理，一律看三分解、不要引净额**：`cash_like`（净）＝ `cash_positive` ＋ `cash_offset`。
DRAM 实测 `cash_like` **−25.27%** = `cash_positive` +13.95% ＋ `cash_offset` −39.22%。
`CASH OFFSET` 是 swap 名目的融资对冲行，**不是持有的现金**（脚本的 `OFFSET_MARKERS` 已识别）。
报告写「现金/国库券类合计 −25.27%」对读者毫无意义——要写就写 `cash_positive`，
并把 `cash_offset` 另行说明。

⚠️ **而且 AV 的现金口径与官方持仓表会正面冲突。** 实测 2026-09-07 的 LYTE：
AV 给 `cash_positive` **−3.78%**、`weight_total` 63.94%；而 `rating-rules.md` 正文
已核过的**官方**持仓表是 **现金/抵押品 +39.2%**（T-Bill 36.0 ＋ 货币基金 3.2）、
净股票曝险 99.0%、行项目加总 138.2%。**按口径红线三，冲突时以官方为准**——
LYTE 的持仓结构数字继续引正文那一套，不要改用 AV 的。
