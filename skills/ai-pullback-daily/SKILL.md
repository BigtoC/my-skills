...
- **精简版**：Slack markdown（*粗体* + • 项目符号，不用表格）。**字符上限一律以
  `references/output-format.md` 原文为准（≤3000），本文件不另立数字**——早先这里写「≤5000」
  又同时说「以原文为准」，两个数字并存等于没有上限。溢出处置见该文件的编者注：
  **必出行与三桶的每一行都不可删**，46 档标的的宽触发日必然超过 3000（实测 2026-09-05：4,493 字符），
