...
> #### 编者注（2026-09-14）·上表新增两组腿，以及「周一四项」少了一项
>
> **信号 1 多两条观察腿（每日）**：`BAMLC0A0CM`（IG OAS）、`BAMLC0A4CBBB`
> （BBB OAS），与原有的 `BAMLH0A0HYM2` 同为**每日更新**，走 `fred.sh --credit`
> 三条腿同日对齐。**两条新腿不计入任何触发计数**，口径与陷阱见
> `signals-a-macro.md` 信号 1 的编者注。
>
> **信号 32 从「每周一」移到「每日更新」**，并从一条腿变三条：`DGS10`（名目）、
> `DFII10`（实质，原有）、`T10YIE`（盈亏平衡），走 `fred.sh --rates`。
> 三者皆为**每日更新**（营业日）。因此上面那行「每周一额外执行 H. 周一附加
> **四项**」现在实际是**三项**（31 / 33 / 34）；信号 32 每天都在。
> 它仍然**不计入 30 个信号、不参与触发计数**。
>
> ⚠️ **两组的历史深度差很多，不要混为一谈**：`DGS10` / `DFII10` / `T10YIE` 的
> `cosd` 正常生效（分别可回到 **1962** / 2003 / 2003——2026-09-14 实测
> `cosd=1950-01-01` 回 16,878 / 6,181 / 6,182 笔）；而 **ICE BofA 家族
> （`BAML*`）在 `fredgraph.csv` 只回滚动 3 年并静默钳住 `cosd`**——见
> 「已知失效 / 陷阱清单」。校准阈值时这个差别是决定性的。
> 另注：`--rates` 三条腿**同日内连接**后，可用样本一律被 `DFII10` / `T10YIE` 钳在
> 2003 年起——`DGS10` 那 1962 年的深度只有单独取该序列时才拿得到，不要拿它当
> 信号 32 的校准样本长度。
