...
# │ · 历史均值是本脚本唯一的除数：整页 0.00 骗得过上面的结构自检（0 既是     │
# │   最小也是最大），除下去 awk 会以退出码 2 收场，正好撞上「依赖缺失」的   │
# │   码位 → 除数非正数一律当解析错位处理，以 3 退出，绝不当 0 算。          │
...
# 除数护栏：历史均值是下面「当前较均值 %」唯一的除数。
# 上面那道结构自检**挡不住整页 0.00**——0 既满足「第 4 个最小」也满足「第 5 个最大」，
# 五个数一路放行，直到 c/m 让 awk 以「division by zero」致命结束。awk 那个退出码是 2，
# 正好撞上全仓保留给「依赖缺失」的码位：一次解析失败会被读成环境装坏了，
# 而且 stdout 上连一个 ⚪️ 或一份 JSON 都不会留下。
# 均值 ≤0 或根本不是数字，代表抓到的不是 Shiller PE 那一栏 → 与结构自检同族，走 3。
# （量级自检只审当前值，管不到除数：当前值可能仍落在 5–50 内，见下面的 SANITY_OK。）
if ! awk -v m="$MEAN" 'BEGIN{ exit ((m + 0) > 0) ? 0 : 1 }'; then
  warn "⚪️ 信号 28（Shiller CAPE）数据暂缺 —— 页面取到了（HTTP 200），但历史均值解析成「${MEAN}」，不能当除数。"
  warn "   抓到的五个数为：current=${CUR} mean=${MEAN} median=${MEDIAN} min=${MIN} max=${MAX}"
  warn "   均值为 0 或非数字 → multpl.com 版型已变（或回了一个空壳页），抓到的不是 Shiller PE 那一栏。"
  die "「当前较均值」绝不可当 0 算、也不可用旧读数顶替。请人工核对 ${URL}。备援源：currentmarketvaluation.com / gurufocus 的 CAPE 页。" 3
fi
